...
- `looping_curve`：纵向列队垂直入场 → 圆弧绕向远侧（入场 / 离场均保持正切方向）→ 沿出生侧或指定切线离场，同时保持模型朝向移动方向。配置支持两种模式：通过 `aiParams.auto` 仅给出入场锚点、圆心与离场点即可自动生成整条平滑曲线；需要微调时再附加 `entry / arc / exit` 覆盖局部参数。入场段会根据实际出生高度自动延长，保证纵向编队不会在屏外堆叠；关卡波次还可以直接覆写 `aiParams`，无需额外的镜像敌人 ID。
