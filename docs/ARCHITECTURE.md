...
- `looping_curve`：纵向列队垂直入场 → 圆弧绕向远侧（入场/离场均保持正切方向）→ 沿出生侧或指定切线离场，同时保持模型朝向移动方向。配置支持两种模式：
  - **自动切线模式**（推荐）：通过 `aiParams.auto` 仅给出入场锚点、圆心与离场点，系统自动求解正切点并生成整条平滑曲线
  - **手动精调模式**：使用 `entry / arc / exit` 三段参数逐项微调；与 `auto` 同时存在时，手动字段会覆盖自动推算结果
  - 入场段会根据实际出生高度自动延长，保证纵向编队不会在屏外堆叠
  - 关卡波次可直接覆写 `aiParams`，无需额外的镜像敌人 ID
