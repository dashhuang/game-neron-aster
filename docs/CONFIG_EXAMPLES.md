...
- [敌人配置示例](#敌人配置示例)
  - [六边形敌人](#六边形---基础敌人)
  - [三角形敌人](#三角形---快速敌人)
  - [环形编队敌人](#三角形---环形编队敌人自动切线)
  - [波次覆写示例](#关卡波次覆写示例向右向下离场)
- [武器配置示例](#武器配置示例)
- [关卡配置示例](#关卡配置示例)
...
> 同一波次中既可以混用不同敌人 ID，也可以为同一个敌人提供多套 `aiParams`。向下离场只需把 `exitPoint` 改为 `{ "x": 360, "y": 1500 }` 并把 `minArcDeg` 调整为 `315`，即可让队列在圆弧右侧折向屏幕底部离开。`enemy_test` 关卡的 4 波样例分别演示了左上入右出（基础）、右上入左下出、底部入右上出以及上入左下出的组合，全部通过波次覆写实现。
