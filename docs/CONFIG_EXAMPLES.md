...
> 同一波次中既可以混用不同敌人 ID，也可以为同一个敌人提供多套 `aiParams`。向下离场只需把 `exitPoint` 改为 `{ "x": 360, "y": 1500 }` 并把 `minArcDeg` 调整为 `315`，即可让队列在圆弧右侧折向屏幕底部离开。敌人测试关卡的 4 波样例分别演示了左入右出（默认）、右入左下出、左下入右上出以及上入左下出的组合。
