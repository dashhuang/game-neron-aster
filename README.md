...
* `enemy_test` 关卡保留两波 8 敌人的纵向编队：
  * 第一波从左上入场，沿出生侧右移离场，列队中第 2、6 架为射手
  * 第二波从右上镜像入场，绕行后朝左下离场，同样仅第 2、6 架射击
  * 第三波再次从左侧进入，完整绕圈后沿屏幕底部离场，仅第 2、6 架射击
  * 用于验证 `looping_curve` 行为的多种离场方向、镜像路径与部分射击敌人的组合
  * 波次配置示例使用对象形式覆写 `enemies[].aiParams`，无需在 `enemies.json` 中维护额外 ID
