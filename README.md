...
* **箭头群**（快、脆、成群扑杀）
  - 基础型：快速俯冲
  - 狙击型：装备瞄准武器
  - **环形尖兵**：垂直列队下潜，正切进入圆弧后绕向远侧，再沿出生侧水平切线飞离屏幕（`looping_curve` AI，可通过敌人配置的 `aiParams.entry/arc/exit` 或锚点式 `entryAnchor/exitAnchor` 调整入场高度、圆弧半径和离场角度，路径会自动保持平滑，并依据实际出生高度延展入场段，避免纵向队列在屏外堆叠）
* **菱形追踪者**（精英、追踪导弹）
  - 装备追踪导弹，持续威胁
  - 距离触发射击
* 旋臂块（分裂/旋转）- 未实现
