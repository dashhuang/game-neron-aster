...
* `enemy_test` 关卡包含四波 8 敌人的纵向编队：
  * 第 1 波：左侧入场 → 绕圈后向右侧离场，第 2、6 架为射手（基础样例）
  * 第 2 波：右侧入场 → 绕圈后向左下离场，第 2、6 架为射手（波次覆写 `aiParams` 示例）
  * 第 3 波：左下入场 → 绕圈后向右上离场，第 2、6 架为射手
  * 第 4 波：上方入场 → 绕圈后向左下离场，第 2、6 架为射手
  * 通过关卡波次覆写 `enemies[].aiParams`，无需在 `enemies.json` 中维护额外 ID，就能快速试验不同进出方向
