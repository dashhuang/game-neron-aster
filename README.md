...
* **箭头群**（快、脆、成群扑杀）
  - 基础型：快速俯冲
  - 狙击型：装备瞄准武器
  - **环形尖兵**：垂直列队下潜，正切进入圆弧后绕向远侧，再沿出生侧或指定切线飞离屏幕。`looping_curve` AI 支持 `aiParams.auto` 自动求解入场 / 圆心 / 离场的正切点，也可继续使用 `entry / arc / exit` 覆盖局部参数；系统会自动保持路径平滑，并依据实际出生高度延展入场段，避免纵向队列在屏外堆叠。默认配置（`triangle_loop` / `triangle_loop_right`）已使用 `auto` 描述，只需改写圆心与离场坐标即可镜像出不同方向。
* **菱形追踪者**（精英、追踪导弹）
  - 装备追踪导弹，持续威胁
  - 距离触发射击
* 旋臂块（分裂/旋转）- 未实现
