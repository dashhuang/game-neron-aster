...
- **快速开始**：[开发者指南](docs/DEVELOPER_GUIDE.md) - 环境搭建、常见任务
- **架构设计**：[ARCHITECTURE.md](docs/ARCHITECTURE.md) - ECS 架构、系统列表、数据流
- **配置手册**：
  - [DATA_CONFIG.md](docs/DATA_CONFIG.md) - 敌人、武器配置格式与参数说明
  - [LEVEL_CONFIG.md](docs/LEVEL_CONFIG.md) - 关卡、波次、编队配置
  - [CONFIG_EXAMPLES.md](docs/CONFIG_EXAMPLES.md) - 完整配置示例
- **颜色规范**：[COLOR_DESIGN.md](docs/COLOR_DESIGN.md) - 颜色使用规范（必读）
...
* **当前布局（2025-11-14）**：
  * 结构：战舰核心 → 4 个母节点（进攻/防御/成长/机动）→ 对应子分支
  * 节点示例：进攻系包含 `ATK → (CRIT / PIERC / SPEED)`，成长系包含 `XP → (MAG-R → MAG-S / LOOT / CHOICE → PITY)`
  * 资源规则：4 个母节点统一使用核心能量；`战舰核心` 使用星级点数；`稀有技能保底（PITY）` 首次用时间棱晶，后续用核心能量
* **视觉语言**：
...
* **数据来源**：`src/data/talents/talentTree.ts`（TypeScript 常量）
  * 关键字段：`category / position / connections / costs`
  * `costs` 为数组，可描述“按等级切换资源”的场景（如 PITY 首次用时间棱晶，后续用核心能量）
  * UI 根据 `costs` 决定节点颜色与 Tooltip 中的资源显示
...
* **箭头群**（快、脆、成群扑杀）
  - 基础型：快速俯冲
  - 狙击型：装备瞄准武器
  - **环形尖兵**：垂直列队下潜，正切进入圆弧后绕向远侧，再沿出生侧或指定切线飞离屏幕。`looping_curve` AI 支持 `aiParams.auto` 自动求解入场 / 圆心 / 离场的正切点，也可继续使用 `entry / arc / exit` 覆盖局部参数；系统会自动保持路径平滑，并依据实际出生高度延展入场段，避免纵向队列在屏外堆叠。现在只需要一个 `triangle_loop` / `triangle_loop_shooter` 敌人，通过关卡波次覆写 `aiParams` 就能切换为右侧镜像、向下离场等多种轨迹。
* **菱形追踪者**（精英、追踪导弹）
  - 装备追踪导弹，持续威胁
  - 距离触发射击
* 旋臂块（分裂/旋转）- 未实现
...
* `enemy_test` 关卡包含四波 8 敌人的纵向编队：
  * 第 1 波：左上入场 → 绕圈后向右侧离场，第 2、6 架为射手（基础配置）
  * 第 2 波：右上入场 → 绕圈后向左下离场，第 2、6 架为射手（波次覆写示例）
  * 第 3 波：底部入场 → 绕圈后向右上离场，第 2、6 架为射手（波次覆写示例）
  * 第 4 波：上方入场 → 绕圈后向左下离场，第 2、6 架为射手（波次覆写示例）
  * 通过波次覆写 `enemies[].aiParams`，无需在 `enemies.json` 中维护额外 ID，快速试验不同进出方向
...
- **线性关卡1**（linear_01）- wave_script 模式
  - 46 个波次，整体保持 2 秒节奏，末段收束为 1.5 秒冲刺段
  - 第 25 波起每波敌人数在原基础额外 +2，持续拉高场面密度
  - 武器倍数从 ×1.2 递增至 ×2.0，逐段叠加射速/伤害
  - **支持通关流程**：最后敌人消失后自动通关
